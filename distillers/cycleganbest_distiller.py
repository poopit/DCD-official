...
class CycleganBestDistiller(BaseCycleganBestDistiller):
    ...
    def calc_DCD_loss(self):
        ...
        for (netT,netS,Tact,Sact) in zip(self.netTs,self.netSs,Tacts,Sacts):
            self.T = netT(Tact.detach())
            self.S = netS(Sact)
            
            # --Discriminator version
            self.netD_teacher_A(self.S)
            Slogits = [self.Dacts[key] for key in sorted(self.Dacts.keys())]
            self.netD_teacher_A(self.T)
    ...
    def backward_G_Student(self):
        # self.pretrain_discrim = torch.nn.Sequential(*(list(self.netD_teacher_A.children())[:-1])).to(self.device)
        self.fake = self.netG_teacher_A(self.real_A)
        self.Sfake = self.netG_student(self.real_A)
        teacher_image = self.fake.detach()

        pred_fake = self.netD_teacher_A(self.fake)
        ...
        self.loss_G_student = 0.0
        self.loss_G_student = self.loss_G_style + self.loss_G_feature + self.loss_G_gan
        
        if self.opt.lambda_CD:
            self.loss_G_CD = self.calc_DCD_loss() *self.opt.lamda_CD
    ...
    def backward_linear(self):
        self.loss_linear = -1.0* self.D_Loss()
        self.loss_linear.backward()
    ...
    def evaluate_student_model(self, step):
        ...
        for i, data_i in enumerate(tqdm(eval_dataloader, desc='Eval    ', position=2, leave=False)):
            ...
            with torch.no_grad():
                ...
                self.Tfake_B = self.netG_teacher_A(self.real_A)
            ...
            for j in range(len(self.image_paths)):
                ...
                if cnt < 10:
                    ...
                    for k,(S_in, T_in) in enumerate(zip(trans_S, trans_T)):
                        ...
                        S_img = util.tensor2im(S)
                        T_img = util.tensor2im(T)
                        student_file_path = 'student_stage_' + str(k+1)
                        teacher_file_path = 'teacher_stage_' + str(k+1)
                        util.save_image(S_img , os.path.join(save_dir,student_file_path,'%s.png') % name,
                                    create_dir=True)
                        util.save_image(T_img , os.path.join(save_dir,teacher_file_path,'%s.png') % name,
                                    create_dir=True)
                        
                    util.save_image(Sfake_im, os.path.join(save_dir, 'Sfake', '%s.png' % name),
                                    create_dir=True)
                    util.save_image(Tfake_im, os.path.join(save_dir, 'Tfake', '%s.png' % name),
                                    create_dir=True)
                    util.save_image(input_im, os.path.join(save_dir, direction, 'input', '%s.png') % name,
                                    create_dir=True)
                cnt += 1
