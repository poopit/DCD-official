...
class BaseCycleganBestDistiller(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.pretrain_D = networks.define_D(opt.output_nc, opt.ndf, opt.netD,
                                                         opt.n_layers_D, opt.norm,
                                                         opt.init_type, opt.init_gain, self.gpu_ids)
        ...
        self.pretrain_D.eval()
        self.extrac_D = ['model.10']
        ...
        if self.opt.lambda_CD:
            for i, n in enumerate(self.mapping_layers[self.opt.teacher_netG]):
                ...
                if 'resnet' in self.opt.teacher_netG:
                    netT = self.build_feature_connector(3, 4 * ft)
                    netS = self.build_feature_connector(3, 4 * fs)
                elif i == 0:
                    netT = self.build_feature_connector(3, 2 * ft)
                    netS = self.build_feature_connector(3, 2 * fs)
                elif i == 1:
                    netT = self.build_feature_connector(3, 8 * ft)
                    netS = self.build_feature_connector(3, 8 * fs)
                elif i == 2:
                    netT = self.build_feature_connector(3, 16 * ft)
                    netS = self.build_feature_connector(3, 16 * fs)
                else:
                    netT = self.build_feature_connector(3, 4 * ft)
                    netS = self.build_feature_connector(3, 4 * fs)
        ...
        T_params = [self.netG_teacher_A.parameters()]
        T_params.append(self.netG_teacher_B.parameters())
        ...
        self.optimizer_G_teacher = torch.optim.Adam(itertools.chain(*T_params), lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    def setup(self, opt, verbose=True):
        ...
        if self.opt.lambda_CD > 0:
            ...
            add_hook(self.netD_teacher_A, self.Dacts, self.extrac_D)
